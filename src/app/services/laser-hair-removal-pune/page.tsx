...
import type { LaserHairRemovalContent } from './types';

const LaserHairRemovalClientPage = () => {
  // Type assertion for the imported JSON data
  const typedContentData = contentData as LaserHairRemovalContent;
      ...
      <section className="py-12 md:py-20 px-4 md:px-8 bg-[color:var(--color-light-background)]">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col gap-8 md:gap-12">
            ...
            >
              <h2
                className={`text-3xl md:text-[40px] leading-tight md:leading-[48px] text-[color:var(--color-dark-text)] font-semibold mb-8 transform transition-all duration-700 delay-200 ease-out ${
                  isVisible("areas-header")
                    ? "translate-y-0 opacity-100"
                    : "translate-y-5 opacity-0"
                }`}
              >
                {typedContentData.areasWeTreat.title}
              </h2>
            </div>
            <div
              id="areas-grid"
            ...
            >
              {typedContentData.areasWeTreat.items.map((area, index) => (
                <div
                  key={index}
...
export default LaserHairRemovalClientPage;