...
const serviceFormSchema = z.object({
    ...
    data: z.object({
        ...
        postCare: z.object({
            ...
            info: z.object({
                title: z.string().optional(),
                items: z.array(z.string().min(1, "Info item cannot be empty")).optional(),
            }).optional(),
        }).optional(),
        faq: z.object({
            title: z.string().optional(),
...
export default function AddServicePage() {
    ...
    const { fields: infoFields, append: appendInfo, remove: removeInfo } = useFieldArray({
        control: form.control,
        name: "data.postCare.info.items" as any,
    });
    ...
    const onSubmit = async (values: ServiceFormValues) => {
        try {
            ...
            if (!cleanedData.postCare?.downtime?.title && !cleanedData.postCare?.info?.title) {
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-8">
                            ...
                            {/* Info */}
                            <div className="space-y-4">
                                <h4 className="text-lg font-medium">Care Information</h4>
                                <FormField
                                    control={form.control}
                                    name="data.postCare.info.title"
                                    render={({ field }) => (
                                        <FormItem>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <h5 className="text-sm font-medium">Info Items</h5>
                                        <Button
                                            type="button"
                                            variant="outline"
                                            size="sm"
                                    ...
                                    {infoFields.map((field, index) => (
                                        <div key={field.id} className="flex gap-4 items-center">
                                            <FormField
                                                control={form.control}
                                                name={`data.postCare.info.items.${index}`}
                                                render={({ field }) => (
                                                    <FormItem className="flex-1">
                                                        <FormControl>
                                                            <Input placeholder="Info item" {...field} />
                                                        </FormControl>
                                                        <FormMessage />
                                                    </FormItem>
                                                )}
